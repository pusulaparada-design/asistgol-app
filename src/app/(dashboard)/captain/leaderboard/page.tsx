...
import { getCaptainTeamsStats, getCaptainPlayersLeaderboard } from "@/lib/actions/match";

export default async function LeaderboardPage() {
  const [myTeamsStats, { topScorers, fairPlay }] = await Promise.all([
    getCaptainTeamsStats().catch(() => []),
    getCaptainPlayersLeaderboard().catch(() => ({ topScorers: [], fairPlay: [] })),
  ]);
    ...
    <PageContent>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <Card className="xl:col-span-1">
          <CardHeader title="Golcü Sıralaması" subtitle="Takımlarımın oyuncuları" />
          <div className="divide-y divide-[#F3F4F6]">
            {topScorers.length === 0 && (
              <div className="py-8 text-center text-sm text-[#9CA3AF]">Henüz gol kaydı yok</div>
            )}
            {topScorers.map(({ rank, id, name, teamName, goals }) => (
              <div key={id} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-[#9CA3AF]">{teamName}</div>
                </div>
        ...
        <div className="xl:col-span-2">
          <Card>
            <CardHeader title="Fair Play Sıralaması" subtitle="Takımlarımın oyuncuları — en az kart" />
            {fairPlay.length === 0 ? (
                ...
                {fairPlay.map((r) => (
                  <div key={r.id} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-[#9CA3AF]">{r.teamName}</div>
                    </div>
