...
import { MapPin, Calendar, Trophy, Users, Swords, BarChart2, CheckCircle, XCircle, GitBranch, Settings2, ChevronDown, ChevronUp, Shirt } from "lucide-react";
...
function effectiveStatus(t: Tournament) {
  ...
  if (t.status === "REGISTRATION") {
    if (t.matches.length > 0 && approved >= t.maxTeams)
      return { label: "Fikstür Hazır", variant: "blue" as const };
    if (approved >= t.maxTeams)
      return { label: "Kontenjan Doldu", variant: "orange" as const };
  }
...
function expectedTotalMatches(t: Tournament) {
  ...
  // GROUP_KNOCKOUT — knockout henüz oluşturulmadı, hesapla
  const advancing = (t.advanceCount ?? 2) * t.groups.length;
  const knockoutExpected = advancing > 1 ? (advancing - 1 + (t.thirdPlace ? 1 : 0)) : 0;
  return groupMatches + knockoutExpected;
}
...
export default function TournamentDetailClient({ tournament: t }: { tournament: Tournament }) {
  ...
  const totalExpectedMatches = expectedTotalMatches(t);
  const totalYellow = t.matches.reduce((acc, m) => acc + m.cards.filter(c => c.type === "YELLOW").length, 0);
  const totalRed    = t.matches.reduce((acc, m) => acc + m.cards.filter(c => c.type === "RED").length, 0);
  const totalGoals  = t.matches.reduce((acc, m) => acc + m.goals.length, 0);
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mt-5 grid grid-cols-2 sm:grid-cols-5 gap-3">
            {[
              ...
              { label:"Oynanan Maç",   val:`${playedMatches.length}/${totalExpectedMatches}` },
          ...
          {TABS.map(tb => (
            ...
            >
              ...
              {tb.key === "takimlar" && pendingRegs.length > 0 && (
                <span className="bg-[#EF4444] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">{pendingRegs.length}</span>
              )}
            </button>
          ))}
        </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-6 space-y-6">
        ...
        {tab === "gruplar" && (
          <div className="space-y-4">
            {t.groups.length === 0 ? (
              <Card><div className="p-8 text-center text-sm text-[#9CA3AF]">Henüz grup oluşturulmadı.</div></Card>
            ) : (
              <>
                ...
                {t.groups.filter(g => g.id === activeGroup).map(g => {
                  ...
                  const groupMatches = [...t.matches.filter(m => m.groupId === g.id)].sort((a, b) => {
                  ...
                  return (
                    <div key={g.id} className="space-y-4">
                      ...
                      {groupMatches.length > 0 && (
                        <Card>
                          <CardHeader title="Maçlar" subtitle={`${groupMatches.length} maç`} />
        ...
        {tab === "maclar" && (() => {
          ...
          return (
            <Card>
              <CardHeader
                title="Tüm Maçlar"
                subtitle={`${sorted.length} maç · ${played} oynandı`}
              />
              {sorted.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-[#F3F4F6]">
                  {topScorers.length === 0 ? (
                    <tr><td colSpan={4} className="px-4 py-6 text-center text-sm text-[#9CA3AF]">Henüz gol kaydedilmedi.</td></tr>
                  ) : topScorers.map((p, i) => (
            ...
            <Card>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-[#F3F4F6]">
                  {topCarders.length === 0 ? (
                    <tr><td colSpan={5} className="px-4 py-6 text-center text-sm text-[#9CA3AF]">Henüz kart kaydedilmedi.</td></tr>
                  ) : topCarders.map((p, i) => (
                    <tr key={i} className="hover:bg-[#FAFAFA]">
                      <td className="px-4 py-2.5 text-xs text-[#9CA3AF]">{i+1}</td>
                      <td className="px-4 py-2.5 text-sm font-medium text-[#111827]">{p.name}</td>
...
function TeamTab({ pendingRegs, approvedRegs }: { pendingRegs: Tournament["registrations"]; approvedRegs: Tournament["registrations"] }) {
          ...
          <div className="divide-y divide-[#F3F4F6]">
            {pendingRegs.map(r => {
              ...
              return (
                <div key={r.id}>
                  <div
                    className="flex items-center gap-4 px-5 py-4 cursor-pointer hover:bg-[#FAFAFA] transition-colors select-none"
                    onClick={() => toggle(r.id)}
                  >
        ...
        <div className="divide-y divide-[#F3F4F6]">
          ...
          {approvedRegs.map(r => {
            ...
            return (
              <div key={r.id}>
                <div
                  className="flex items-center gap-4 px-5 py-3 cursor-pointer hover:bg-[#FAFAFA] transition-colors select-none"
